...
Hardware controller for Behringer X-Air digital mixers built on an ESP microcontroller.

The project provides a standalone controller with physical controls (encoders, buttons, faders) and a graphical display to control mixer parameters over OSC via WiFi.
...
The controller connects to the mixer via WiFi and communicates using OSC messages, which are used by Behringer X-Air mixers.

Build

The firmware is intended to be built for an ESP microcontroller using a typical embedded toolchain such as:
...
MIT License (or adjust if you use a different one).
